...
class CNNSimulation {
  ...
  setup() {
    ...
    for (const row in rowsCNN) {
      inputValues = rowsCNN[row].arr;
      this.inputValuesRotated = rotateArray(rowsCNN[row].arr);
      ...
      this.hiddenCubes[row] = buildGeometry(`hiddenCubes${row}`, (builder) => {
        for (let i = 0; i < hidden.flattendInput[0].length; i++) {
          ...
          let hiddenElement = this.hiddenLayer[i];
          let colorValue = map(
          ...
          builder.push();
          builder.translate(hiddenElement.x, hiddenElement.y, hiddenElement.z);
          builder.fill(cubeColor);
          canvas3DCNN.noStroke();
          builder.box(5);
          builder.pop();
        }
        ...
        for (let i = 1; i < this.inputValuesRotated.length; i++) {
          let inputElement = this.inputLayer[i - 1];
          let colorValue = map(
            Number(this.inputValuesRotated[i]),
            0,
            255,
            0,
            1
          ...
          builder.push();
          builder.translate(inputElement.x, inputElement.y, inputElement.z);
          builder.fill(cubeColor);
          canvas3DCNN.noStroke();
          builder.box(10);
          builder.pop();
  ...
  draw() {
    ...
    if (rowsCNN && millis() - this.lastProcessedTime >= 700) {
      ...
      inputLayerModel = this.inputCubes[this.currentRow];
      hiddenLayerModel = this.hiddenCubes[this.currentRow];
    ...
    //canvas3DCNN.model(hiddenLayerModel);
    // canvas3DCNN.model(inputLayerModel);
